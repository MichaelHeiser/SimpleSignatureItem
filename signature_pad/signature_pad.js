...
.import "throttle.js" as Throttle
...
function SignaturePad(canvas, options) {
  ...
  if (this.throttle) {
    this._strokeMoveUpdate = Throttle.throttle(SignaturePad.prototype._strokeUpdate, this.throttle);
  } else {
...
SignaturePad.prototype.fromDataURL = function (dataUrl) {
  this._canvas.loadImage(dataUrl)
  this._isEmpty = false;
};
...
SignaturePad.prototype._createPoint = function (x, y, time) {
  const rect = {x: 0, y: 0, width: this._canvas.width, height: this._canvas.height, top: 0, right: this._canvas.width, bottom: this._canvas.height, left: 0}
...
SignaturePad.prototype._addPoint = function (point) {
  ...
  if (points.length > 2) {
    ...
    const c3 = tmp.c1;
    const curve = new Bezier.Bezier(points[1], c2, c3, points[2]);
  ...
  for (let i = 0; i < pointGroups.length; i += 1) {
    ...
    if (group.length > 1) {
      for (let j = 0; j < group.length; j += 1) {
        ...
        const point = new Point.Point(rawPoint.x, rawPoint.y, rawPoint.time);
